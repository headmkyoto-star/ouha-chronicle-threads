...
GITHUB_RAW_BASE = "https://raw.githubusercontent.com/headmkyoto-star/ouha-chronicle-threads/main/"
GITHUB_API_BASE = "https://api.github.com/repos/headmkyoto-star/ouha-chronicle-threads/contents/"
...
def pick_menu():
    ...
    try:
        ...
        with open(_MENU_STATE_PATH, "w") as f:
            _json_dedup.dump({"menu": chosen, "date": _dt_dedup.date.today().isoformat()}, f, ensure_ascii=False, indent=2)
...
def generate_post():
    ...
    prompt = f"""京都河原町のリラクゼーションサロン「おうはクロニクル」の若い女性セラピストとして、Threadsの短い営業投稿を作成してください。
...
def is_rest_day_jp():
    """投稿は火・土以外の週5回。火・土ならTrue（JST基準）"""
    import datetime
    today = (datetime.datetime.utcnow() + datetime.timedelta(hours=9)).date()
    if today.weekday() in (1, 5):
        print(f"😴 {today} は火・土なので投稿お休み")
        return True
    return False
